...
// Keep legacy category and renamed-story links working at the root.
const resolvePost = cache(async (slug: string) => {
  ...
  if (slug === "pulsar-n125-launched-in-nepal") {
    permanentRedirect("/pulsar-n125-nepal");
  }
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    publisher: SITE.name,
    alternates: seoAlternates(`/${post.slug}`),
    openGraph: {
      type: "article",
      title: post.title,
      description,
